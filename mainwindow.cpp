...
<<<<<<< HEAD
    ...
    // 清理页面
    if (m_mode1Page) { stack->removeWidget(m_mode1Page); m_mode1Page->deleteLater(); m_mode1Page = nullptr; }
    if (m_mode2Page) { stack->removeWidget(m_mode2Page); m_mode2Page->deleteLater(); m_mode2Page = nullptr; }
    if (m_mode3Page) { stack->removeWidget(m_mode3Page); m_mode3Page->deleteLater(); m_mode3Page = nullptr; }
...
void MainWindow::onAIDemoStart()
{
    ...
    // === 【修改点 1：修复背景】 ===
    // 1. 必须先清除之前设置的图片 Palette
    QPalette pal = this->palette();
    pal.setBrush(QPalette::Window, QBrush(Qt::NoBrush)); // 清除画刷
    setPalette(pal);

    // 2. 然后再设置背景色 (使用深色背景，与 mode_ai.ui 保持一致)
    this->setStyleSheet(
        "background: qradialgradient(cx:0.5 cy:0.5 radius:1.4,"
        "fx:0.5 fy:0.3,"
        "stop:0 #0d3742,"
